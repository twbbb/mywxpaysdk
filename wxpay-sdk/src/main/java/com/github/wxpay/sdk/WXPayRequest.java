...
public class WXPayRequest
{
			...
			int readTimeoutMs, boolean useCert) throws Exception
	{
		...
		// config.getMchID()); // TODO: 很重要，用来检测 sdk 的使用情况，要不要加上商户信息？
...
//		}
		String result = requestOnce(WXPayConstants.DOMAIN_API, urlSuffix, uuid, data, connectTimeoutMs, readTimeoutMs, useCert);
			...
			int readTimeoutMs, boolean autoReport) throws Exception
	{
		 ...
		 * (ConnectTimeoutException ex) { exception = ex; WXPayUtil.getLogger().
			...
			boolean autoReport) throws Exception
	{
		 ...
		 * format("connect timeout happened for domain {}, try to use {}",
		 * domain, this.primaryDomain)); shouldRetry = true; } catch
		 * (SocketTimeoutException ex) { exception = ex; shouldRetry = false; }
		 * catch (Exception ex) { exception = ex; shouldRetry = false; }
		 * 
